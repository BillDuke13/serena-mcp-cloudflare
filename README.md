...
This repository packages a **Cloudflare Workers + Containers deployment wrapper**
for [Serena](https://github.com/oraios/serena). It is intended for operators who
want a hosted Serena MCP endpoint without modifying Serena's core runtime.

## Upstream Serena

- Upstream project: `oraios/serena`
- License: MIT (this repository also uses MIT)
- Scope of this repository: deployment, routing, auth, and operational docs
...
- `cloudflare/src/index.ts` – Worker auth, routing, and MCP proxy logic
- `cloudflare/bin/start-serena-mcp.sh` – container entrypoint with local-state + R2 snapshot sync
- `Dockerfile.cloudflare` – container image build for Serena MCP on Cloudflare Containers
- `wrangler.toml` – Cloudflare deployment configuration (template-safe defaults)
- `cloudflare/README.md` – copy of this deployment guide for subdirectory navigation
